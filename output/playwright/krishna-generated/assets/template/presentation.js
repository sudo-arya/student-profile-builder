(() => {
  ...
  document.querySelectorAll('[data-section-type="education"] .semantic-entry, [data-section-type="experience"] .semantic-entry').forEach(entry => {
    ...
    } else if (headingPlace && strongText && /\d{4}|present|current/i.test(strongText)) {
      place = headingPlace;
      period = strongText;
    } else {
  ...
  const contact = document.querySelector('[data-section-type="contact"] .markdown-body');
  if (contact) {
    const paragraphs = [...contact.querySelectorAll(':scope > p')];
    const intro = paragraphs.find(p => p.querySelector('a[href^="mailto:"]'));
    const email = intro?.querySelector('a[href^="mailto:"]');
    if (intro && email) {
      const action = email.cloneNode(true);
      action.className = 'contact-email';
      email.remove();
      intro.textContent = intro.textContent.trim().replace(/\s*email\s*[,.]?\s*$/i, '').replace(/[,.]?\s*$/, '.');
      intro.after(action);
    }

    const metadata = paragraphs.find(p => p.querySelectorAll('strong').length > 1);
    if (metadata) {
      ...
      [...metadata.childNodes].forEach(node => {
        if (node.nodeType === Node.ELEMENT_NODE && node.tagName === 'STRONG') {
          ...
          current.className = 'contact-detail';
          const label = document.createElement('span');
          label.textContent = node.textContent.replace(/:$/, '');
          current.appendChild(label);
          details.appendChild(current);
        } else if (current && !(node.nodeType === Node.ELEMENT_NODE && node.tagName === 'BR')) {
          current.append(node.cloneNode(true));
        }
      });
      metadata.replaceWith(details);
    }
  }
})();
